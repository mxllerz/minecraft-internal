...
// Forward declarations for legacy OpenGL functions used in Minecraft
// This avoids conflicts between <GL/gl.h> and the internal ImGui loader.
extern "C" __declspec(dllimport) void __stdcall glBlendFunc(unsigned int sfactor, unsigned int dfactor);
extern "C" __declspec(dllimport) void __stdcall glHint(unsigned int target, unsigned int mode);

// Legacy OpenGL defines for Minecraft compatibility
...
extern "C" {
    __declspec(dllimport) void __stdcall glPushAttrib(unsigned int mask);
    __declspec(dllimport) void __stdcall glPopAttrib(void);
    __declspec(dllimport) void __stdcall glPushClientAttrib(unsigned int mask);
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ...
    glDisable(GL_ALPHA_TEST); // Fix for Minecraft pixelated text
    glDisable(GL_LIGHTING);   // Fix for Minecraft text lighting
    glDisable(GL_FOG);        // Ensure no fog is applied to UI
    glDisable(GL_COLOR_LOGIC_OP); // Ensure no logic ops are applied
    glDisable(GL_FRAMEBUFFER_SRGB); // Disable gamma correction for UI (fixes "crunchy" AA)
    glDisable(GL_MULTISAMPLE);      // Disable MSAA for UI (fixes blurry edges)
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // Force pixel alignment for texture uploads
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Force Minecraft to use GL_LINEAR for this specifically
